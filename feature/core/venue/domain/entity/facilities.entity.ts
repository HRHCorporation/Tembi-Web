...
export default class Facilities {
  ...
  constructor({ id, icon, name_eng, name_ind, description_eng, description_ind }: { id: number; icon?: string; name_eng: string; name_ind: string; description_eng?: string; description_ind?: string; }) {
    this.id = id;
    this.icon = icon;
  ...
  getDescription(language: "id" | "en"): string | undefined {
    return language === "id" ? this.description_ind || undefined : this.description_eng || undefined;
  }

  ...
  return (
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200">
        ...
        <div
          className={`p-8 text-center flex flex-col items-center ${currentStyle.bg}`}
        >
          <div
            className={`w-20 h-20 ${currentStyle.iconBg} rounded-full flex items-center justify-center mb-6 shadow-inner`}
          >
            <Icon
              className={`w-10 h-10 ${currentStyle.iconColor} stroke-[2.5]`}
            />
          </div>
          <h3
            className={`text-2xl font-serif font-bold mb-3 ${currentStyle.titleColor}`}
          >
            {title}
          </h3>

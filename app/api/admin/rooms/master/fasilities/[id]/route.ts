...
export async function GET(
    ...
    try {
        ...
        const [rows] = await dbWeb.query<FasilityRow[]>(  // ← ganti any
            `SELECT * FROM mstr_fasilities WHERE id = ? LIMIT 1`,
            [numericId]
        );
        if (!rows.length) {
...
export async function PUT(
    ...
    try {
        ...
        await dbWeb.query(
            `UPDATE mstr_fasilities SET name_ind = ?, name_eng = ?, icon = ?, updated_at = NOW() WHERE id = ?`,
...
export async function DELETE(
    ...
    try {
        ...
        await dbWeb.query(
            `DELETE FROM mstr_fasilities WHERE id = ?`,
            [Number(id)]
        );
        return NextResponse.json({ success: true });
    } catch (error) {

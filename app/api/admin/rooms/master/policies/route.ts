...
export async function POST(request: NextRequest) {
    try {
        ...
        await dbWeb.query(
            `
            INSERT INTO mstr_room_policies (name_ind, name_eng, icon, type, created_by, created_at)
            VALUES (?, ?, ?, ?, ?, NOW())
            `,
            [name_ind, name_eng, icon, type, createdBy]
        );
        return NextResponse.json({ success: true, message: "Kebijakan berhasil ditambahkan" });
    } catch (error) {
        console.error(error);

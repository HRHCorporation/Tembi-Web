...
import { getFasilityById, updateFasility } from "../_services/fasilities.service";

export function useEditFasilityRoom(id: string) {
    ...
    async function fetchDetail() {
        try {
            ...
            const result = await getFasilityById(Number(id));
            if (result.success) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (!description_ind) {
            newErrors.description_ind = "Deskripsi fasilitas (Indonesian) harus diisi";
            hasError = true;
        }
        if (!description_eng) {
            newErrors.description_eng = "Deskripsi fasilitas (English) harus diisi";
            hasError = true;
        }
        setErrors(newErrors);
        if (hasError) return;
        ...
        try {
            ...
            const result = await updateFasility(Number(id), {
                name_ind: name_ind,
                name_eng: name_eng,
                icon: icon,

...
function SuccessContent() {
  ...
  const generatePDF = async () => {
    ...
    doc.text(
      `Check-in  : ${format(new Date(booking.checkInDate), "dd MMM yyyy")}`,
      110,
      80,
    );
    doc.text(
      `Check-out : ${format(new Date(booking.checkOutDate), "dd MMM yyyy")}`,
      110,
      86,
    );
    ...
    if (booking.breakfast > 0) {
      tableRows.push([
        ...
        formatCurrency(
          booking.breakfast * ADDONS_PRICE.breakfast * booking.duration,
        ),
      ]);
    }
    if (booking.extraBed > 0) {
      tableRows.push([
        ...
        formatCurrency(
          booking.extraBed * ADDONS_PRICE.extrabed * booking.duration,
        ),
      ]);
    }
    ...
    doc.text(formatCurrency(booking.totalPrice), 196, finalY + 15, {
      align: "right",
    });
    ...
    doc.text(
      "Thank you for your reservation at Tembi Historical Home.",
      105,
      280,
      { align: "center" },
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9F9F9] pt-28">
        <div className="text-center p-8 bg-white rounded-xl shadow border border-red-100">
          ...
          <button
            onClick={() => router.push("/")}
            className="bg-tembi text-white px-4 py-2 rounded"
          >
            Back to Home
          </button>
        </div>
  ...
  return (
    <main className="min-h-screen bg-[#FCFCFA] pb-20 pt-28 font-sans">
      ...
      <div className="container mx-auto max-w-200 px-4 -mt-16 relative z-10">
          ...
          <div className="p-8">
            ...
            <div className="space-y-3 pt-6 border-t border-dashed border-gray-200">
              <div className="flex justify-between font-medium text-gray-700">
                <span>
                  {booking.roomName} x {booking.duration} nights
                </span>
                <span>
                  {formatCurrency(booking.roomPrice * booking.duration)}
                </span>
              ...
              {booking.breakfast > 0 && (
                <div className="flex justify-between text-gray-600 bg-orange-50 p-2 rounded-lg text-sm">
                  <span className="flex items-center gap-2">
                    <Coffee size={14} /> Extra Breakfast ({booking.breakfast}
                    pax)
                  </span>
                  <span>
                    {formatCurrency(
              ...
              {booking.extraBed > 0 && (
                <div className="flex justify-between text-gray-600 bg-green-50 p-2 rounded-lg text-sm">
                  <span className="flex items-center gap-2">
                    <Bed size={14} /> Extra Bed ({booking.extraBed} units)
                  </span>
                  <span>
                    {formatCurrency(
                      booking.extraBed *
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4">
          <button
            onClick={generatePDF}
            className="flex items-center justify-center gap-2 px-6 py-3 border-2 border-[#8CA873] text-[#5A6E48] font-bold rounded-xl hover:bg-[#8CA873]/10 transition-colors"
          >
            <Download size={18} /> Download Invoice (PDF)
          </button>
          <button
            onClick={() => router.push("/")}
            className="px-8 py-3 bg-[#3A4D39] text-white font-bold rounded-xl hover:bg-[#2c3b2b] shadow-lg shadow-[#3A4D39]/20 transition-all"
          >
            Back to Home
          </button>
        </div>

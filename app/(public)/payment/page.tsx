...
import { Check, CheckCircle, XCircle, Lock, Loader2 } from "lucide-react";
...
const PaymentStepper = () => (
  <div className="flex items-center justify-center w-full max-w-xl mx-auto mt-8 mb-4 text-sm relative z-10">
    ...
    <div className="w-12 h-0.5 bg-[#8B8055] mx-3"></div>
    <div className="flex items-center gap-2 text-[#8B8055] font-bold">
      <div className="w-6 h-6 rounded-full bg-[#8B8055] text-white flex items-center justify-center text-xs">
        2
      </div>
      <span>Payment</span>
    </div>
...
function PaymentContent() {
  ...
  const bookingData = useMemo(() => {
    ...
    const serviceFee = Math.round((basePrice + totalAddonsCost) * 0.05);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.message || data?.error || "Payment processing failed",
        );
      }
  ...
  const cardStyle =
    "bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative z-10";
  const inputStyle =
    "w-full px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:border-[#8B8055] focus:ring-1 focus:ring-[#8B8055] transition-all text-sm";

  return (
    <main className="min-h-screen bg-[#EFF1F0] font-sans text-gray-800 pb-20 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-0 overflow-hidden">
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-350 h-350 bg-[#FAFAF9] shadow-2xl opacity-100"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 flex flex-col gap-6">
            <div className={`${cardStyle} p-0 overflow-hidden`}>
              ...
              <div className="p-6">
                <h2 className="font-bold text-lg font-serif mb-4 text-gray-900">
                  Booking Summary
                </h2>
                <div className="space-y-3 text-sm text-gray-600 mb-6">
                  <div className="flex justify-between pb-2 border-b border-dashed border-gray-200">
                    ...
                    <span className="font-bold text-gray-800">
                      {format(parseISO(checkIn), "EEE, MMM dd, yyyy")}
                    </span>
                  </div>
                  <div className="flex justify-between pb-2 border-b border-dashed border-gray-200">
                    <span>Check-out</span>
                    <span className="font-bold text-gray-800">
                      {format(parseISO(checkOut), "EEE, MMM dd, yyyy")}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    ...
                    {addons.extrabed > 0 && (
                      <div className="flex justify-between text-gray-700">
                        <span className="flex items-center gap-1">
                          Extra Bed
                          <span className="text-xs bg-[#8B8055] text-white px-1.5 rounded-md">
                            {addons.extrabed}x
                          </span>
                        </span>
                    ...
                    {addons.breakfast > 0 && (
                      <div className="flex justify-between text-gray-700">
                        <span className="flex items-center gap-1">
                          Breakfast
                          <span className="text-xs bg-orange-500 text-white px-1.5 rounded-md">
                            {addons.breakfast}x
                          </span>
                        </span>
            ...
            <div className={cardStyle}>
              <h4 className="font-bold mb-4 text-lg font-serif">
                Cancellation Policy
              </h4>
              <ul className="space-y-3 text-sm text-gray-600">
                <li className="flex items-start">
                  <CheckCircle className="w-4 h-4 mr-3 mt-0.5 text-green-600 shrink-0" />
                  <span>Free cancellation until 48h before check-in.</span>
                </li>
                <li className="flex items-start">
                  <CheckCircle className="w-4 h-4 mr-3 mt-0.5 text-orange-500 shrink-0" />
                  <span>50% refund within 48h.</span>
                </li>
                <li className="flex items-start">
                  <XCircle className="w-4 h-4 mr-3 mt-0.5 text-red-600 shrink-0" />
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="text-center md:text-left">
              ...
              <div className="flex items-baseline gap-2 justify-center md:justify-start">
                <p className="text-3xl font-bold text-[#8B8055] font-serif">

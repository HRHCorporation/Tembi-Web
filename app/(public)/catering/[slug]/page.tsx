...
export default function CateringDetailPage() {
  ...
  const { foodSlug, foodSlugLoading, foodSlugError, refreshSlug } =
    useFoodContext();
  ...
  return (
    <main className="min-h-screen w-full bg-white overflow-x-hidden">
            ...
            <div className="mb-10 flex flex-wrap gap-4">
              <div className="flex items-center gap-3 rounded-full bg-white/20 px-5 py-2.5 text-white backdrop-blur-md border border-white/10 transition hover:bg-white/30">
                <div className="relative h-5 w-5">
                  <Image
                    src={foodSlug?.image || "/images/icons/group-green.png"}
                    alt="Pax Icon"
                    fill
                ...
                <span className="font-medium">
                  {foodSlug?.getMinimumPax(language) || "Loading..."}
                </span>
              </div>
              <div className="flex items-center gap-3 rounded-full bg-white/20 px-5 py-2.5 text-white backdrop-blur-md border border-white/10 transition hover:bg-white/30">
                <div className="relative h-5 w-5">
                ...
                <span className="font-medium">
                  {foodSlug?.getHourService(language) || "Loading..."}
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <div className="flex gap-16">
              <div>
                <h3 className="font-serif text-4xl font-bold text-[#96A66D]">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <div className="mx-auto mb-16 max-w-2xl text-center">
            ...
            <h2 className="mb-6 font-serif text-4xl font-bold text-[#4A3B32] lg:text-5xl">
              {foodSlug?.getTitleMenu(language) || "Loading..."}
            </h2>
            <p className="text-lg text-gray-600">
              {foodSlug?.getDescriptionMenu(language) || "Loading..."}
            </p>
          </div>
          <div className="flex flex-wrap justify-center gap-8">
            {foodSlug?.menus && foodSlug.menus.length > 0 ? (
              foodSlug.menus.map((menu) => {
                const iconConfig = menu.getIconConfig?.();

                const computedIcon = iconConfig
                  ? iconConfig.icon
                  : (menu.icon ?? "");
      ...
      {foodSlug?.packages && foodSlug.packages.length > 0 ? (
        <BuffetPricing
          title={
            language === "id"
          ...
          subtitle={
            language === "id" ? "Pilih Paket Anda" : "Choose Your Package"
          }
          description={
            language === "id"
              ? "Pilih paket yang sempurna untuk acara Anda."
              : "Select the perfect package for your event."
          }
          whatsappNumber="62812345678"
          packages={foodSlug?.packages || []}
          language={language}
        />
      ) : (
        <section className="w-full bg-white py-24">
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="mb-6 font-serif text-4xl font-bold text-[#4A3B32] lg:text-5xl">
                {language === "id"
                  ? "Paket Catering Tidak Tersedia"
                  : "Catering Packages Unavailable"}
              </h2>
              <p className="text-lg text-gray-600">
                {language === "id"
                  ? "Maaf, paket catering untuk layanan ini saat ini tidak tersedia. Silakan hubungi kami untuk informasi lebih lanjut."
                  : "Sorry, catering packages for this service are currently unavailable. Please contact us for more information."}
              </p>
            </div>
          </div>
        </section>

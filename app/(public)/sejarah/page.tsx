...
export default function SejarahPage() {
	...
	const { historyBanner, historyBannerLoading } = useHistoryContext();
	return (
          ...
          <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent z-10" /> */}
					{historyBannerLoading ? (
						...
						<>
							<Image
								src={
									historyBanner?.image || "/images/sejarah/history-hero.webp"
								}
								alt={
									historyBanner?.getTitle(language) ||
									"Tembi Historical Home Background"
								}
								fill
								className="object-cover"
								priority
				...
				<div className="max-w-7xl mx-auto">
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
						<ScrollReveal animation="slideLeft" duration={800}>
							<div className="order-2 lg:order-1">
								...
								<div className="text-gray-600 space-y-6 text-lg leading-relaxed font-light mb-10 text-justify">
									<p>{t.history.intro.subtitle[0]}</p>
									<p>{t.history.intro.subtitle[1]}</p>
								</div>
							</div>

							<div className="grid grid-cols-3 gap-4 border-t border-gray-100 pt-8">
								<div>
									<h3 className="text-3xl font-serif font-bold text-[#9CA389]">
										25+
									</h3>
									<p className="text-xs md:text-sm text-gray-500 mt-1">
										{t.history.intro.item[0]}
									</p>
								</div>
								<div>
									<h3 className="text-3xl font-serif font-bold text-[#9CA389]">
						...
						<div className="relative order-1 lg:order-2">
							...
							<div className="absolute -bottom-10 -left-10 w-48 h-48 bg-[#Eef0eb] rounded-4xl -z-10" />
				...
				<div className="max-w-7xl mx-auto">
					<div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
						<div className="lg:col-span-3 order-2 lg:order-1 flex justify-center lg:justify-start">
							<div className="relative w-full max-w-70 aspect-square rounded-4xl overflow-hidden shadow-2xl -rotate-2 hover:rotate-0 transition-transform duration-500">
								<Image
						...
						<div className="lg:col-span-6 order-1 lg:order-2 text-center lg:px-4">
							<ScrollReveal animation="fadeUp" duration={800} delay={200}>
								...
								<div className="text-gray-600 space-y-6 text-lg leading-relaxed font-light text-justify">
									<p>
										{t.history.founder.desc1[0]}
										<strong>{t.history.founder.desc1[1]}</strong>
										{t.history.founder.desc1[2]}
									</p>
									<p>
										{t.history.founder.desc2[0]}
										<strong>{t.history.founder.desc2[1]}</strong>
										{t.history.founder.desc2[2]}
									</p>
								</div>
							</ScrollReveal>
						</div>
						...
						<div className="lg:col-span-3 order-3 lg:order-3 flex justify-center lg:justify-end">
							<div className="relative w-full max-w-70 aspect-3/4 rounded-4xl overflow-hidden shadow-2xl rotate-2 hover:rotate-0 transition-transform duration-500">
								<Image
							...
							<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
								<div>
									...
									<div className="text-gray-600 space-y-5 font-light leading-relaxed mb-8 text-justify">
										<p>{t.history.living.desc[0]}</p>
										<p>{t.history.living.desc[1]}</p>
									</div>

									<div className="flex gap-4">
										<div className="bg-[#F8F6F1] rounded-xl p-4 w-52 text-center">
											<span className="block text-2xl font-serif font-bold text-[#9CA389]">
												15+
											</span>
											<span className="text-xs text-gray-500 uppercase tracking-wide">
												{t.history.living.item[0]}
											</span>
										</div>
										<div className="bg-[#F8F6F1] rounded-xl p-4 w-52 text-center">
											<span className="block text-2xl font-serif font-bold text-[#9CA389]">
...
const CultureCard: React.FC<CultureCardProps> = ({
	...
	<div className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
		...
		<div className="p-8 flex-1 flex flex-col">
			...
			<p className="text-gray-600 font-light leading-relaxed text-sm">{desc}</p>
		</div>
	</div>
);
...
const ContactCard: React.FC<ContactCardProps> = ({ title, desc, iconSrc }) => (
	<div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 flex flex-col items-start text-left hover:bg-white/20 transition-all duration-300">
		...
		<p className="text-sm text-gray-200 font-light leading-relaxed">{desc}</p>
	</div>
);
